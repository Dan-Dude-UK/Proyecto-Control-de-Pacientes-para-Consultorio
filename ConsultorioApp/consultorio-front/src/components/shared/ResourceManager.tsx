...
export type FieldType = "text" | "email" | "number" | "date" | "boolean" | "textarea";
...
export function ResourceManager<T extends { id: number }>({
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e: Error) => toast.error(e.message || "No se pudo eliminar el registro"),
  });
        ...
        {query.isLoading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/60 text-left text-xs font-semibold tracking-wide uppercase text-muted-foreground">
                <tr>
        ...
        <DialogContent className="max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {fields.map((f) => (
              <div
                key={f.key}
                className={f.type === "boolean" || f.type === "textarea" ? "sm:col-span-2" : ""}
              >
                {f.type === "boolean" ? (
